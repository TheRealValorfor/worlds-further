...
2. **Enable Realistic Ores (3D Ores)** if the content list still shows it disabled. The zip is selected in `options.txt`.
...
5. **Soaring Trader Flyovers** belongs in **Add More Content → Data Packs** once the CurseForge project is approved. Until then the zip is already in `datapacks/` (same on-disk layout CurseForge uses).
...
| **`resourcepacks/`** | Every zip in that folder (3D Ores, Better Ores 3D, ROP, CreateSophBackpacks, Boss Refreshed, Glowing Blocks, Glowix, Fresh Animations, Freshly Modded, Torrezx-Glowy, Cubic Sun & Moon, Dramatic Skys). |
...
| **`datapacks/`** | CurseForge Data Packs folder (`SoaringTraderFlyovers-1.0.0.zip`). Moonlight loads this globally (`config/moonlight-common.toml`). **Skip this and trader planes stay on the mod defaults (Y 220, no avoid).** When the datapack is a real instance addon, the zip still lives here — same as Complementary in `shaderpacks/`. |
...
- `overrides/config/moonlight-common.toml` has `global_datapacks_folder = "datapacks"`
...
Last in the `resourcePacks` list wins. Overlay / CEM packs sit after base textures. Cubic then **Dramatic Skys** last among skies.

Bottom → top: 3D Ores → Better Ores 3D → ROP → CreateSophBackpacks → Boss Refreshed → Glowing Blocks → Glowix → Fresh Animations → Freshly Modded → Torrezx-Glowy → Cubic Sun & Moon → Dramatic Skys.

Continuity is **not** enabled. Do not turn it back on.

## JVM arguments (does **not** ship)

This machine uses **12 GB** locally plus ZGC flags. **Players still get the CurseForge default (often 4 GB)** until they change it. Put this in the project description:
...
Recommended RAM: **8 GB** minimum; **10–12 GB** with Distant Horizons + shaders.

## What `config/` already forces (if ticked)

- Complementary **LOW**, shaders **on**, Iris shadow distance **10**
- DH ~**72** chunks, **BLOCK** near the player, **HIGH** horizontal drop-off, surface-only distant gen, 2 worker threads (Mac-safe). No SSAO/AA (shaders handle that)
- Vanilla render **4**, simulation **8**, graphics **Fancy**
- Mac Retina resolution reduction on; Flywheel lighting **TRI_LINEAR**
...
CurseForge profile **Worlds further**: memory override **on**, 8–12 GB; Additional Java Arguments = contents of `ADDITIONAL_JVM_ARGS.txt`. That lives in `minecraftinstance.json` on this machine only.
